...
def update_timetable():
    ...
    try:
        tt_list = []
        with open(path.join(base_dir, 'log', 'timetable.txt'), encoding='utf-8-sig') as tt:
            for line in tt:
    ...
    try:
        html_list = []
        with open(path.join(base_dir,'log','timetable.html'), encoding='utf-8-sig') as html:
            for line in html:
...
def write_html(tt_list):
    ...
    with open(path.join(base_dir,'log','timetable.html'), 'w+', encoding='utf-8-sig') as tt_html:
...
def by_num_of_sessions(t, first_session_time=None, timetable_only=False, study_length=50, break_length=10):
    ...
    timetable_path = path.join(base_dir, 'log', 'timetable.txt')
...
def ring_bell():
    ...
    # bellObj.play()
    # sleep(bellObj.get_length)

    # ring bell using pyglet
    bell = media.Player()
    x = media.load(bell_path, streaming=False)
    bell.queue(x)
    bell.volume = 0.6
    bell.play()
    sleep(x.duration)
